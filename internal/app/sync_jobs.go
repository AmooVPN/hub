...
func (r *Runner) loadSyncJobList(ctx context.Context, statusFilter, panelFilter string) ([]syncJobListRow, error) {
	query := `SELECT sj.id, COALESCE(p.name, ''), sj.job_type, sj.status, COALESCE(sj.message, ''), COALESCE(sj.finished_at, sj.started_at, sj.created_at) FROM sync_jobs sj LEFT JOIN panels p ON p.id = sj.panel_id`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.PanelName, &item.JobType, &item.Status, &item.Message, &item.When); err != nil {
			return nil, err
		}
...
func (r *Runner) loadSyncJobDetail(ctx context.Context, id int64) (*syncJobListRow, error) {
	row := r.db.QueryRowContext(ctx, `SELECT sj.id, COALESCE(p.name, ''), sj.job_type, sj.status, COALESCE(sj.message, ''), COALESCE(sj.finished_at, sj.started_at, sj.created_at) FROM sync_jobs sj LEFT JOIN panels p ON p.id = sj.panel_id WHERE sj.id = ?`, id)
	var item syncJobListRow
	if err := row.Scan(&item.ID, &item.PanelName, &item.JobType, &item.Status, &item.Message, &item.When); err != nil {
...
func renderSyncJobDetailPage(item *syncJobListRow, appName, adminRole string) string {
	body := `<div class="container py-4 py-lg-5"><div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3"><div><h1 class="h3 mb-1">Sync Job #` + strconv.FormatInt(item.ID, 10) + `</h1><p class="text-body-secondary mb-0">` + html.EscapeString(defaultString(item.PanelName, "No panel")) + `</p></div><a class="btn btn-outline-secondary btn-sm" href="/admin/sync-jobs">Back</a></div><div class="card shadow-sm"><div class="card-body"><dl class="row mb-0"><dt class="col-sm-3">Panel</dt><dd class="col-sm-9">` + html.EscapeString(defaultString(item.PanelName, "-")) + `</dd><dt class="col-sm-3">Type</dt><dd class="col-sm-9">` + html.EscapeString(item.JobType) + `</dd><dt class="col-sm-3">Status</dt><dd class="col-sm-9">` + syncJobStatusBadge(item.Status) + `</dd><dt class="col-sm-3">Message</dt><dd class="col-sm-9">` + html.EscapeString(defaultString(item.Message, "-")) + `</dd><dt class="col-sm-3">When</dt><dd class="col-sm-9">` + html.EscapeString(item.When.Format(time.RFC3339)) + `</dd></dl></div></div></div>`
	return renderAdminShell(appName, adminRole, "sync-jobs", body)
}
